...
/**
 * This class is basic implementation for ICWizardHandler interface. It is
 * independent of managed build system, and, so, almost useless It creates
 * "empty" project with no specific.
 * 
 * Its descendants should overwrite some methods, including createProject() and
 * handleSelection()
 * 
 * This object is created per each Project type on the left pane of New Project
 * Wizard page
 * 
 * It is responsible for: - corresponding line in left pane of 1st wizard page -
 * whole view of right pane - processing preferred items, if any. - providing
 * data for ConfigPage - processing data received from config page
 *
 */
public class CWizardHandler implements Cloneable {
	...
	/** 
	 * added by jwy, see @org.eclipse.cdt.managedbuilder.ui.wizards.MBSWizardHandler
	 * 
	 */
	public void handleSmartSimuAPPSelection() {
	}
	...
	/**
	 * @return null if data is consistent else returns error message
	 */
	...
	/**
	 * Asks handler to update its data according to preferred list. Usually,
	 * marks preferred toolchains somehow (icon, font etc)
	 * 
	 * @param prefs
	 *            - list of strings (preferred Toolchain IDs)
	 */
	 ...
	 * 
	 * @param proj
	 *            - simple project to be used as base
	 * @param defaults
	 ...
	 *            - true if called from 1st Wizard page
	 * @param onFinish
	 *            - true when the project creation is performed on finish. false
	 *            -otherwise false means that the project created is actually a
	 *            temporary one that can be removed in case cancel is pressed
	 * 
	 * @throws CoreException
	 */
	 ...
	 *            - true if called from 1st Wizard page
	 * @param monitor
	 *            - progress monitor to track the creation process
	 * @throws CoreException
	 * @since 5.1
	 */
	...
	/**
	 * 
	 * @return true if settings were changed since last call to saveState()
	 */
	 ...
	 *            - affected project
	 * @param created
	 *            - true if the project was created before
	 */
	 ...
	 * 
	 * @param data
	 *            - Wizard Item data to be added as child to current Wizard item
	...
	/**
	 * Called after project creation in order to apply template-specific
	 * settings.
	 * 
	 * Can be called 2 or more times: - each time when user presses <Advanced
	 * Settings> button - when user presses <Finish> button. If <Adv. settings>
	 * were not invoked, called once.
	 * 
	 * Since the project is cleared before each method call, no need to check
	...
	/**
	 * Called after project creation in order to apply settings from custom
	 * pages.
	 * 
	 * Can be called 2 or more times: - each time when user presses <Advanced
	 * Settings> button - when user presses <Finish> button. If <Adv. settings>
	 * were not invoked, called once.
	 *
	 * Since the project is cleared before each doCustom() call, no need to
	 * check whether it has been called before.
	 * 
	 * @param prj
	 *            - affected project.
	 */
